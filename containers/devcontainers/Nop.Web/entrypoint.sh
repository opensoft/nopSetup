...
SETUP_SCRIPT="$SCRIPT_DIR/setupContainer.sh" # Path relative to entrypoint script

if [ -f "$SETUP_SCRIPT" ]; then
    echo "Attempting to run setupContainer script: $SETUP_SCRIPT"
    ...
    if [ $EXIT_CODE -eq 0 ]; then
        echo "setupContainer script ($SETUP_SCRIPT) completed successfully."
        echo "Container is running, development tools ready..."
    else
        echo "-----------------------------------------------------"
        echo "ERROR: setupContainer script ($SETUP_SCRIPT) failed with exit code $EXIT_CODE."
    ...
    echo "setupContainer script not found at '$SETUP_SCRIPT', skipping..."
fi
